...
function oneFold(input) {
  const grid = makeGrid(input[0]);
  const folds = input[1]
    .split("\n")
    .map((d) => d.replace("fold along ", "").split("="));
  const gridAfterFoldOne = fold(grid, folds[0][0], Number(folds[0][1]));
  console.log(gridAfterFoldOne);
  return gridAfterFoldOne.reduce(
    (a, v) => (a += v.reduce((ax, vx) => (ax += vx === "#" ? 1 : 0), 0)),
    0
  );
}
